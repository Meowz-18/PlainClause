...
| 5 | **BM25-lite clause retrieval for Q&A** | `lib/ai/retrieval.ts` (new), `tests/unit/retrieval.test.ts` (new) | All clauses sent to model on every `/api/ask` call — 10–120 KB input per question regardless of relevance | Only top-k relevant clauses sent (BM25 + synonym expansion + heading bonus). Small docs (≤15 clauses) and broad queries pass through unchanged. Previously cited clauses always preserved. **50–85% input token reduction** for targeted questions on large docs. | **Zero** — 125/125 tests pass (22 new retrieval tests) |
| 6 | Integrate retrieval into `/api/ask` route | `app/api/ask/route.ts` | `buildAskPrompt()` received full `clauses[]` on every call (Groq, Anthropic, and offline paths) | `retrieveRelevantClauses()` filters clauses before prompt construction. Cited clause IDs extracted from conversation history for citation-verifiability invariant. | **Zero** — 125/125 tests pass, tsc clean |
| 7 | DRY refactor: offline fallback uses shared retrieval | `app/api/ask/route.ts` | ~70 lines of inline keyword scoring duplicated from retrieval logic | Replaced with `scoreClausesForQuery()` + `isBroadQuery()` calls to the shared retrieval module. Net -69 lines. | **Zero** — identical behaviour, better synonym coverage via SYNONYM_INDEX |
